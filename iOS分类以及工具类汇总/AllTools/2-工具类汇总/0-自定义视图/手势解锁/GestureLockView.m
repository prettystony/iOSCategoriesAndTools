...
#define KGestureLineSelectedColor [UIColor colorWithHexString:@"#69bebf"]
#define KGestureLineErrorColor SETCOLOR(223, 66, 48, 1.0)
const static NSUInteger kNumberOfNodes = 9;
const static NSUInteger kNodesPerRow = 3;
const static CGFloat kNodeDefaultWidth = 80;
const static CGFloat kNodeDefaultHeight = 80;
...
- (void)_lockViewInitialize{
	...
	//DLOG(@"contentView.width == %f,contentView.height == %f",self.contentView.frame.size.width,self.contentView.frame.size.height);
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([self.selectedButtons count] > 0) {
        ...
        NSUInteger selectedCount = [self.selectedButtons count];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    self.normalGestureNodeImage = [UIImage imageNamed:@"gesture_node_normal"];
    self.selectedGestureNodeImage = [UIImage imageNamed:@"gesture_node_selected"];
    self.lineColor = KGestureLineSelectedColor;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    self.trackedLocationInContentView = CGPointMake(kTrackedLocationInvalidInContentView, kTrackedLocationInvalidInContentView);
    [self setNeedsDisplay];
//    [self performSelector:@selector(dismissRedrawView) withObject:self afterDelay:0.5];
}
